Meteor.methods({
	...
	addResponses: function(data){
		...
		var usr = Users.findOne({"_id": data.userId, "studentId": data.studentId});
		if (!usr)
	...
	setTiming: function(data) {
		...
		var sess = Sessions.findOne({"sessionId": data.sessionId});
		Sessions.update({"sessionId": data.sessionId}, {$set: {clickerData: {responses: sess.clickerData.responses? sess.clickerData.responses: [], countDownSetAt: new Date(), maxSeconds: data.maxSecs}}});
	}
