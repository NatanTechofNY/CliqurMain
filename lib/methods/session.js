	...
	createSession: function(data) {
		...
		if(Meteor.isServer){
			...
			if(Sessions.find().count() !== 2176782336) {
				...
				else if(!Usrs.findOne({"_id": data.sessionOwnerId}))
				...
				var obj = {
					...
					clickerData: {
						...
						sets: [{question: "", responses: []}]
					}
				};
				if (typeof data.pin === "string") {obj.pin = data.pin; obj.hasPin = true;};
				if (typeof data.longitude === "number" && typeof data.latitude === "number") {
					obj.sessLoc = {};
					obj.sessLoc.longi = data.longitude;
					obj.sessLoc.lati = data.latitude;
				};
				if (typeof data.ranger === "number" && data.ranger >= 500 && data.ranger <= 5000) {
					obj.maxLocRange = data.ranger;
				};
				Sessions.insert(obj);
